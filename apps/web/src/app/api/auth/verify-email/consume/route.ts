...
export async function POST(request: Request) { try { assertSameOrigin(request); assertLocalAccountAuthorityAllowed(); await enforceRateLimit(`verify-email-consume:ip:${clientAddress(request)}`, 20, 60 * 60_000); return ok(await verifyEmail(tokenInput.parse(await jsonBody(request)).token)); } catch (error) { return apiError(error); } }
